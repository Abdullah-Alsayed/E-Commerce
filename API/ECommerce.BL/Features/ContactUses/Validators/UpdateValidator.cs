...
public class CreateContactUsValidator : AbstractValidator<CreateContactUsRequest>
{
    ...
    )
    {
        ...
        RuleFor(req => req.Subject)
            .NotEmpty()
            .WithMessage(x =>
                $"{_localizer[Constants.EntityKeys.Subject]} {_localizer[Constants.MessageKeys.IsRequired]}"
            )
            .NotNull()
            .WithMessage(x =>
                $"{_localizer[Constants.EntityKeys.Subject]} {_localizer[Constants.MessageKeys.IsRequired]}"
        ...
        RuleFor(req => req.Message)
            .NotEmpty()
            .WithMessage(x =>
                $"{_localizer[Constants.EntityKeys.Message]} {_localizer[Constants.MessageKeys.IsRequired]}"
            )
            .NotNull()
            .WithMessage(x =>
                $"{_localizer[Constants.EntityKeys.Message]} {_localizer[Constants.MessageKeys.IsRequired]}"
